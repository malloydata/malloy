...
export class Case extends ExpressionDef {
  ...
  getExpression(ns: NamespaceStack): ExprValue {
    ...
    if (this.value) {
      const v = this.value.getExpression(ns);
      dependents.push(v);
    ...
    for (const c of this.choices) {
      const when = c.when.getExpression(ns);
      const then = c.then.getExpression(ns);
    ...
    if (this.elseValue) {
      const elseValue = this.elseValue.getExpression(ns);
