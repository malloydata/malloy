...
`databasePath` (file), `workingDirectory` (string), `motherDuckToken` (secret), `additionalExtensions` (string — comma-separated, factory parses to array), `readOnly` (boolean), `shareable` (boolean), `setupSQL` (text, advanced). The remaining properties — `securityPolicy`, `allowedDirectories`, `enableExternalAccess`, `lockConfiguration`, `autoloadKnownExtensions`, `autoinstallKnownExtensions`, `allowCommunityExtensions`, `allowUnsignedExtensions`, `tempFileEncryption`, `threads`, `memoryLimit`, `tempDirectory`, `extensionDirectory`, `shareableAttachAlias`, `shareableLockSafety` — are all `advanced: true` (security policy, extension policy, resource tuning, and shareable compatibility).

When `shareable: true` (and `databasePath` is a local file), the DuckDB connection binds its primary database to `:memory:` and brackets file access with `ATTACH`/`USE` in `setupOnce()` and `DETACH` in `idle()`. `shareableAttachAlias` defaults to the historical `"malloy_db"`, preserving BuildIDs and historical shareable views which refer to that alias. Explicit `"auto"` mode omits `AS`, discovers the natural catalog from the newly added `database_oid`, and fixes issue #2984 for files whose persisted views refer to that natural name. Other strings are quoted custom aliases. Renaming a database file changes its natural catalog; persisted views which still refer to the old name require that old name as an explicit custom alias. Only disappearance of the recorded OID proves a failed `DETACH` took effect. Natural and custom identity modes receive distinct connection digests; omitted and explicit `"malloy_db"` remain equivalent. A custom alias is required when the natural name conflicts with DuckDB's built-in `memory`, `system`, or `temp` catalogs; catalog-qualified views which depend on one of those reserved names cannot use shareable mode.

Native local-file connections participate in a versioned, realm-wide physical-target broker keyed by canonical path and `(device,inode)`. It grants an exclusive fencing token, hands shareable ownership off in FIFO order after draining complete logical operations, and never releases the token until `DETACH` or terminal native close has a verified postcondition. Yield requests carry the exact lease epoch and form an admission barrier, so a stale callback cannot act on a reacquired target and an owner's queued backlog cannot overtake a waiter. A stream paused at a yielded-row safepoint is cooperatively returned for handoff; its next `next()` rejects with `DuckDBStreamLeaseRevokedError` so a partial result cannot look complete. A running native pull is allowed to reach that safepoint rather than being interrupted. Consumers must exhaust, abort, return, or close streams they no longer need.

Shareable acquisition is cancellable and bounded; external OS-lock conflicts use bounded retry with jitter. A missing writable target is first built and fully closed under a private sibling directory, then published with `link(2)` as an atomic no-replace commit before DuckDB ATTACH opens it. This removes the otherwise unprovable “missing path became an arbitrary inode” window. It requires same-filesystem hard-link support. If the filesystem cannot provide it, both lock-safety modes throw `DuckDBAtomicPublicationUnavailableError`; Malloy does not fall back to an overwriting rename, a partially visible copy, or an unfenced direct create. Pre-created databases remain usable. A host crash in the tiny link-before-unlink interval can leave the hidden staging hard link behind; the next shareable open sees `nlink > 1` and fails closed until an operator verifies and removes the orphan staging directory.

Path replacement and multiply-linked files fail closed in every shareable mode. `shareableLockSafety` defaults to `"best-effort"` for patch compatibility: it bypasses only the conservative `statfs` filesystem-type denylist while retaining inode identity, hard-link, lifecycle, and broker fences. `"strict"` also rejects generic FUSE mounts and known distributed filesystems, but remains a preflight rather than a proof: an unknown filesystem type or an overlay backed by remote storage cannot be proven local from portable Node APIs. Best-effort mode is not a distributed lock and can corrupt under uncoordinated cross-host writers. Strict deployments should enforce a host-level local-filesystem allowlist or use the dedicated out-of-process broker described below.

Malloy rejects shareable native local-file opens from Node worker threads with `DuckDBUnsafeExecutionRealmError`: each worker has a separate JavaScript realm while traditional POSIX locks are associated with their shared process, so the realm broker cannot fence that topology. Ordinary non-shareable local-file connections retain their legacy worker behavior, but deliberately skip the realm-local broker there and therefore have no cross-realm Malloy fence. `:memory:` and remote connections are unaffected.

An independent process or DuckDB CLI cannot signal this in-memory broker, so the host must call `idle()` before granting that uncooperative peer access; there is deliberately no timeout-based lock stealing. No Malloy-managed connection may be mixed with any direct third-party DuckDB native handle for the same file in the same process: such a handle does not participate in the broker, and process-associated `fcntl` close semantics can invalidate the broker's assumptions even on the main thread. Worker and VM realms add another gap because they do not share the JavaScript broker. Finally, pathname `stat` before and after open cannot provide the same hostile-race proof as an fd-bound `fstat`/`openat2` native helper. Strict arbitration for these topologies requires a dedicated out-of-process DuckDB broker which owns the only native handle and serves requests over IPC.

The `:memory:` primary survives a shareable `idle()`. Data-dependent schema cache is invalidated at each successful ownership handoff so external changes cannot be hidden by stale state. Legacy `manifestTemporaryTable()` results remain valid materialized snapshots in that private in-memory primary; their names include the attachment generation, so a fresh manifest after handoff cannot silently reuse an older snapshot. Only effective shareable native connections expose `runSQLWithTemporaryTable(sql, buildConsumerSQL, options)`: it performs CREATE and consumption under one lifecycle lease and normally drops its unique snapshot immediately; a failed DROP remains quarantined for terminal cleanup. Callers such as `QueryModel.searchIndex()` may explicitly request `temporaryTableCache: "connection-generation"`; that form reuses a bounded LRU of immutable snapshots within one attachment generation and atomically replaces a stable TEMP table after handoff. Its consumer is restricted to one prepared `SELECT`, preventing DDL/DML from renaming or otherwise escaping cleanup. Direct native and WASM retain deterministic `manifestTemporaryTable()` reuse. `QueryModel.searchIndex()` selects the scoped path only when that optional capability is a function. `EXPLAIN ANALYZE` joins explicit transaction control, ATTACH/DETACH/USE, and prepared-statement lifecycle SQL in throwing `DuckDBShareableUnsupportedSQLError`, because DuckDB executes the statement wrapped by the otherwise EXPLAIN-typed command. `readOnly: true` is honored via `(READ_ONLY)` on the ATTACH so it scopes the real file, not the writable in-memory primary. Legacy non-shareable `idle()` remains a no-op while the connection is healthy: disconnecting its native connection while higher layers retain native-backed state previously caused `SIGSEGV`/`bad_weak_ptr`; terminal `close()` is the safe cleanup boundary. If a prior terminal close failed, a later `idle()` retries that quarantined cleanup rather than reporting false success.
...
interface ConnectionPropertyDefinition {
  ...
  advanced?: boolean;
  // Security-sensitive string properties may require exact literals so an
  // invalid or unresolved overlay cannot be silently dropped into a weaker
  // default. Factories still validate the resulting value independently.
...
| `idle()` | Connection is logically valid; releasable backend resources are released. State which may be stale after an external ownership handoff is invalidated. Next operation transparently reattaches. | Per-operation pause in long-lived hosts (VS Code, MCP servers) so other writers can claim resources during idle gaps. |
...
- Make sure `setupSQL` and any other init-time work replays cleanly on reattach. Preserve schema cache only when the backend can prove the data source did not change while released; otherwise invalidate it at the ownership-generation boundary.
- An implementation which can receive concurrent `idle()` and query calls must drain or serialize in-flight operations before releasing native resources. Otherwise it must document that the host supplies that synchronization.
...
- Backends may also implement `runSQLWithTemporaryTable(sql, buildConsumerSQL, options)` to create, consume, and promptly discard a bounded-lifetime TEMP result in one logical operation
...
DuckDB has a unique pattern: a static `activeDBs` map groups compatible non-shareable connections by a configuration-sensitive share key. Multiple compatible `DuckDBConnection` instances share one `DuckDBInstance` but each get their own connection handle. The instance is closed only when the last connection to it calls terminal `close()`. This is not traditional pooling — `isPool()` returns `false`.

Non-shareable `DuckDBConnection.idle()` is a no-op. It does not remove a live connection from `activeDBs`, because closing or disconnecting the native handle while higher layers retain native-backed state caused the #2793 `SIGSEGV`/`bad_weak_ptr` regression. Shareable connections instead own private `:memory:` instances and use the physical-target broker plus ATTACH/DETACH lifecycle described above.

`idle()` is also a no-op for `:memory:` databases — closing the instance would silently destroy in-memory state.
