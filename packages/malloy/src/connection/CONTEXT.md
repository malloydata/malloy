...
`authClient` is a different credential path, not a fourth way to supply a key: a google-auth `AuthClient` the host mints and supplies through an overlay — impersonation, workload identity federation, a proxied credential — forwarded into `new BigQuerySDK({...})`. The SDK then resolves the project id through it rather than through ambient credentials, so a connection setting it should also set `billingProjectId`.
