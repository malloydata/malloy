...
`projectId` (string), `serviceAccountKeyPath` (file), `serviceAccountKey` (json), `authClient` (opaque, `source: 'overlay'`, `mustHaveValue`), `location` (string), `maximumBytesBilled` (string, advanced), `timeoutMs` (string, advanced), `billingProjectId` (string, advanced), `setupSQL` (text, advanced)
`authClient` is a google-auth `AuthClient` supplied by a host overlay — impersonation, workload identity federation, a proxied credential — and is forwarded into `new BigQuerySDK({...})`. The SDK then resolves the project id through it rather than through ambient credentials, so a connection setting it should also set `billingProjectId`.
