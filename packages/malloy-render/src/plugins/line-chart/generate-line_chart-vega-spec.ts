...
export function generateLineChartVegaSpecV2(
  ...
  const spec: Spec = {
    ...
    scales: [
      {
        name: 'xscale',
        type: (xIsDateorTime || settings.mode === 'yoy') ? 'time' : 'point',
        domain: settings.mode === 'yoy'
          ? // For YoY mode, use a full year domain from the reference year 2000
            [new Date(2000, 0, 1).valueOf(), new Date(2000, 11, 31).valueOf()]
          : shouldShareXDomain
          ? xIsDateorTime
          ...
          ...((xIsDateorTime || settings.mode === 'yoy')
            ? {
                labels: {
                  enter: {
                    text: {
                      signal: settings.mode === 'yoy'
                        ? // For YoY mode, format as month/day or appropriate granularity without year
                          `timeFormat(datum.value, '${getYoYTimeFormat(xField.isTime() ? xField.timeframe : undefined)}')`
  ...
  if (hasSeries) {
    ...
    spec.legends!.push({
      ...
      title: settings.mode === 'yoy' ? 'Year' : (seriesField ? seriesField.name : ''),
      orient: 'right',
      ...legendSettings,
      values:
        isDimensionalSeries && shouldShareSeriesDomain && seriesSet
  ...
  const mapMalloyDataToChartData: MalloyDataToChartDataHandler = data => {
    ...
    data.rows.forEach(row => {
      ...
      if (settings.mode === 'yoy') {
        ...
        if (localSeriesSet.size >= maxSeries && !localSeriesSet.has(yoyData.year)) {
  ...
  return {
    ...
    getTooltipData(item, view) {
      ...
      if (['x_hit_target', 'ref_line_targets'].includes(markName)) {
        ...
        const title = (xIsDateorTime || settings.mode === 'yoy')
          ? x === NULL_SYMBOL
            ? NULL_SYMBOL
            : settings.mode === 'yoy'
            ? // For YoY mode, format the normalized date appropriately
              getYoYTooltipTitle(new Date(x), xField.isTime() ? xField.timeframe : undefined)
            : renderTimeString(new Date(x), xField.isDate(), xField.isTime() ? xField.timeframe : undefined)
          : x;
      ...
      if (item.datum && ['ref_line_targets'].includes(getMarkName(item))) {
        ...
        const title = (xIsDateorTime || settings.mode === 'yoy')
          ? itemData.x === NULL_SYMBOL
            ? NULL_SYMBOL
            : settings.mode === 'yoy'
            ? getYoYTooltipTitle(new Date(itemData.x), xField.isTime() ? xField.timeframe : undefined)
            : renderTimeString(
                new Date(itemData.x),
                xField.isDate(),
