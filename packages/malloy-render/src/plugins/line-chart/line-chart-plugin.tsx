...
export const LineChartPluginFactory: RenderPluginFactory<LineChartPluginInstance> =
  {
    ...
    create: (field: Field, pluginOptions?: unknown, modelTag?: Tag): LineChartPluginInstance => {
      if (!field.isNest()) {
        throw new Error('Line chart: must be a nest field');
      }

      const lineChartOptions = pluginOptions as LineChartPluginOptions | undefined;
      ...
      try {
        settings = getLineChartSettings(field, undefined, lineChartOptions?.defaults, modelTag);
      } catch (error) {
