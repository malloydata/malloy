...
export function getBarChartSettings(
  ...
  // Normalize tag to viz format - single source of truth
  ...
  // Get the viz tag for accessing properties
  const vizTag = normalizedTag.tag('viz')!; // Safe to use ! since we just verified it exists
