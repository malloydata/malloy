...
function registerLanguages(monacoGlobal, testConfig: MonarchTestConfig) {
    for (const language of testConfig.embeddedLanguages) {
        monacoGlobal.languages.register(
            {id: language.id});
        monacoGlobal.languages.setMonarchTokensProvider(language.id, language.definition);
        // monacoGlobal.languages.setLanguageConfiguration(language.id, language.configuration);
    }
    monacoGlobal.languages.register(
        {id: testConfig.language.id});
    monacoGlobal.languages.setMonarchTokensProvider(testConfig.language.id, testConfig.language.definition);
...
function setupEditor(monacoGlobal, theme: Monaco.IStandaloneThemeData) {
    ...
    const editor = monacoGlobal.editor.create(
...
export function generateMonarchTokenizations(monacoGlobal, testConfig: MonarchTestConfig): TestItem[][] {
    ...
    const colorMap = editor._themeService.getColorTheme().tokenTheme.getColorMap();
    for (const lines of testConfig.testInput) {
        const block = lines.join('\n');
        const actualTokens: Token[][] = monacoGlobal.editor.tokenize(block, testConfig.language.id);
        const cleanedTokens: TestItem[] = actualTokens.map((lineTokens, index) => {
            return {
                line: lines[index],
                tokens: lineTokens.map((lineToken) => {
                    const tokenColorId = editor._themeService.getColorTheme().getTokenStyleMetadata(lineToken.type).foreground;
                    return {
