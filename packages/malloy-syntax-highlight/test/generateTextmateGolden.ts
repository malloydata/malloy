...
import {Registry, parseRawGrammar, INITIAL, IRawTheme, IGrammar, IToken} from 'vscode-textmate'
...
function initializeLanguageMap(config: GoldenTestConfig): Record<string, string> {
...
function initializeRegistry(languageMap: Record<string, string>, theme: IRawTheme) {
    ...
    const registry = new Registry({
        ...
        loadGrammar: (scopeName) => 
            readFile(languageMap[scopeName])
                .then((data: any)=> parseRawGrammar(data.toString(), languageMap[scopeName])),
    });
    return registry;
}
...
function constructLineGolden(line: string, full: IToken[], binary: Uint32Array, colorMap: string[]): TestItem {
...
function tokenizeMultilineDefinitions(grammar: IGrammar, registry: Registry, definitions: string[][]): TestItem[][] {
    ...
    for (let i = 0; i < definitions.length; i++) {
        ...
        for (let j = 0; j < definitions[i].length; j++) {
            ...
            blockGolden.push(constructLineGolden(line, full.tokens, binary.tokens, registry.getColorMap()));
...
function writeGoldens(goldens: TestItem[][], outputPath: string) {
    const outputTemplate = `export default ${util.inspect(goldens, {depth: null})};`
    fs.writeFileSync(outputPath, outputTemplate, 'utf-8');
}

export async function generateLanguageGolden(configPath: string, outputPath: string, testDefinitions: string[][]) {
    ...
    registry.loadGrammar(config.language.scopeName)
        .then((grammar) => {
            if (grammar) {
                const goldens = tokenizeMultilineDefinitions(grammar, registry, testDefinitions);
