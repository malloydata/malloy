...
`@malloydata/db-athena` connects Malloy to [Amazon Athena](https://aws.amazon.com/athena/) running engine version 3. The dialect is **experimental**: a model that uses it must start with `##! experimental.dialect.athena`. This first version runs scalar queries — grouping, aggregation, joins, filters, time — and declares nests, records and arrays unsupported (see below).
...
| --- | --- | --- |
| Nests (`nest:`) | `GetQueryResults` returns a compound value as Presto's text, `{a=1, b=x}`, which nothing reads back. Carrying nests as JSON is a change of its own. | `supportsNesting` is false: a nest is a located compile error, `'athena' does not support nested queries`. |
| `all()`, `exclude()` and `each`, which share the nest machinery | The same. | The same located compile error. |
| Record and array literals | The same. | No translator gate covers a literal, so the dialect's throw surfaces as an internal compiler error that names the feature: `Athena dialect does not support record literals: a compound value arrives from the API as text`. |
| Reading a table's or SQL block's own `row` or `array` column | The same. | The column is `sql native` (`readsNestedData`, `supportsArraysInData` and `compoundObjectInSchema` are false) and its value is that text. |
| `map` and `varbinary` columns | No Malloy type. | `sql native`. |
| A bigint or decimal past 2^53 | The text is exact; Malloy reads a JavaScript number (`supportsBigIntPrecision` is false). | The nearest double. |
...
- Every cell is text, typed by `ColumnInfo.Type`; a NULL is an absent value; a `decimal` carries its precision and scale beside a bare `decimal`; a 32-bit float is spelled `float` here and `real` elsewhere; a compound column is typed only as `row`, `array` or `map`.
...
- The parser rejects `QUALIFY`; `version()` is not a function, and every execution reports `EngineVersion.EffectiveEngineVersion`.
...
- Glue lowercases every identifier and resolves a reference case-insensitively.
