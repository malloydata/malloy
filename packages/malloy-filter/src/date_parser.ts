...
import {
    DateRange, DatePrefix, DateMoment, DateMomentNow, DateMomentInterval, DateMomentNumberInterval,
    DateMomentNumberUnit, DateMomentNumber, DateTimeUnit, DateWeekday, DateMomentIntervalOperator, DateMomentNumberIntervalOperator,
    DateMomentNumberUnitOperator, DateMomentNumberOperator, DateMomentNowOperator,
    DateClause, Clause
} from './clause_types'
...
export class DateParser extends BaseParser {
    ...
    private tokenize(): void {
        ...
        let specialWords: SpecialToken[] = [
            { type: 'UNITOFTIME', value: /^(second|minute|hour|day|week|month|quarter|year)s?$/i, ignoreCase: true },
            { type: 'DAYOFWEEK', value: /^(monday|tuesday|wednesday|thursday|friday|saturday|sunday)$/i, ignoreCase: true },
        ...
        const params: TokenizerParams =
            { trimWordWhitespace: true, splitOnWhitespace: true, specialSubstrings, specialWords };
    ...
    private matchAndMerge(types: string, tokens: Token[], output: Token[]): boolean {
        const idx = this.index;
        const matchedTokens = Tokenizer.matchTypes(types, tokens, idx);
        if (matchedTokens) {
            output.push({ type: 'MOMENT:' + types, value: '', values: matchedTokens, startIndex: matchedTokens[0].startIndex, endIndex: matchedTokens[matchedTokens.length - 1].endIndex });
    ...
    public parse(): FilterParserResponse {
        ...
        while (this.index < this.tokens.length) {
            ...
            if (token.type === ',') {
                if (prefix) {
                    errors.push({ message: 'Invalid ' + prefix, startIndex: token.startIndex, endIndex: token.endIndex });
                }
                this.index++;
                ...
                errors.push({ message: 'Invalid token ' + token.value, startIndex: token.startIndex, endIndex: token.endIndex });
                prefix = undefined;
                this.index++;
            }
        }
    ...
    // LAST|DAYOFWEEK
    private static createMomentInterval(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentInterval {
        const operator: DateMomentIntervalOperator = tokens[0].type as DateMomentIntervalOperator;
        const unit: DateTimeUnit | DateWeekday = tokens[1].value as DateTimeUnit | DateWeekday;
        let moment: DateMomentInterval = { operator, unit };
        if (prefix) { moment.prefix = prefix; }
        return moment;
    }

    // LAST|NUMBER|UNITOFTIME
    private static createMomentNumberInterval(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentNumberInterval {
        const type0 = tokens[0].type;
        const operator: DateMomentNumberIntervalOperator = type0 === 'LAST' ? 'LASTN' : 'NEXTN';
    ...
    private static createMomentNumberIntervalAgo(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentNumberInterval {
    ...
    private static createMomentNumberIntervalFromNow(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentNumberInterval | undefined {
    ...
    // NUMBER|UNITOFTIME
    private static createMomentNumberUnit(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentNumberUnit {
    ...
    // DATE  DATE|TIME
    private static createMomentNumber(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentNumber {
        const operator: DateMomentNumberOperator = (tokens.length == 2) ? 'DATETIME' : 'DATE';
    ...
    private static createMomentNow(prefix: DatePrefix | undefined, tokens: Token[]): DateMomentNow {
        const operator: DateMomentNowOperator = tokens[0].type as DateMomentNowOperator;
        let moment: DateMomentNow = { operator };
        if (prefix) { moment.prefix = prefix; }
        return moment;
    }

    private static createMomentFromToken(prefix: DatePrefix | undefined, token: Token): DateMoment | undefined {
    ...
    private isMatchingToken(position: number, value: string, exactMatch: boolean): boolean {
    ...
    private handleRange(clauses: Clause[]): boolean {
        if (this.isMatchingToken(this.index, 'MOMENT', false) &&
            ...
            const startMoment: DateMoment | undefined = DateParser.createMomentFromToken(undefined, this.tokens[this.index]);
            const endMoment: DateMoment | undefined = DateParser.createMomentFromToken(undefined, this.tokens[this.index + 2]);
            const operator: 'TO' | 'FOR' = this.tokens[this.index + 1].type as 'TO' | 'FOR';
            this.index += 3;
            if (startMoment === undefined || endMoment === undefined) {
                return false;
            }
            const dateRange: DateRange = { start: startMoment, operator, end: endMoment };
    ...
    private handleMoment(prefix: DatePrefix | undefined, clauses: DateClause[]): boolean {
        const token: Token = this.getNext();
        if (token.type.startsWith('MOMENT')) {
            const clause: DateMoment | undefined = DateParser.createMomentFromToken(prefix, token);
            this.index++;
            if (clause === undefined) {
                return false;
            }
