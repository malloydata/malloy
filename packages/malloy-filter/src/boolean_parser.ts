...
export class BooleanParser extends BaseParser {
    ...
    private tokenize(): void {
        ...
        const params: TokenizerParams =
            { trimWordWhitespace: true, splitOnWhitespace: true, specialSubstrings, specialWords: specialWords };
    ...
    public parse(): FilterParserResponse {
        ...
        while (this.index < this.tokens.length) {
            ...
            if (token.type === ',') {
                this.index++;
            } else if (token.type == 'NULL' || token.type == 'TRUE' || token.type == 'FALSE' || token.type == 'NOTNULL') {
