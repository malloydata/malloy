import { NumberCondition, NumberRange, NumberOperator, NumberRangeOperator, Clause } from './clause_types'
...
export class NumberSerializer extends BaseSerializer {
    ...
    // NumberOperator = '<=' | '>=' | '!=' | '=' | '>' | '<';
    private static numberConditionToString(operator: NumberOperator, value: number | null): string {
        if (value === null) {
    ...
    private static getNegatedType(operator: NumberRangeOperator): NumberRangeOperator {
        switch (operator) {
            case '<':  return '>=';
            case '<=': return '>';
            case '>':  return '<=';
    ...
    private static rangeToString(clause: NumberRange): string {
        const negated: string = clause.startOperator === '<' || clause.startOperator === '<=' ? '!=' : '';
        const startOperator = (negated) ? NumberSerializer.getNegatedType(clause.startOperator) : clause.startOperator;
        const endOperator = (negated) ? NumberSerializer.getNegatedType(clause.endOperator) : clause.endOperator;
        const leftBracket: string = startOperator === '>' ? '(' : '[';
        const rightBracket: string = endOperator === '<' ? ')' : ']';
        return negated + leftBracket + clause.startValue + ', ' + clause.endValue + rightBracket;
    }
    ...
    private static clauseToString(clauses: Clause[]): string {
        ...
        for (const clause of clauses) {
            if ('operator' in clause && clause.operator === 'range') {
                ...
                result += ', ';
            } else if ('operator' in clause && NumberSerializer.isNumberOperator(clause.operator)) {
