import {
    DateRange, DateMoment, DateMomentInterval, DateMomentNumberInterval,
    DateMomentNumberUnit, DateMomentNumber, Clause
} from './clause_types'
...
export class DateSerializer extends BaseSerializer {
    ...
    private static dateMomentToString(operator: string, clause: Clause): string {
        if (operator === 'NOW' || operator === 'TODAY' || operator === 'YESTERDAY' || operator === 'TOMORROW') {
            const custom: DateMoment = clause as DateMoment;
            return (custom.prefix) ? custom.prefix + ' ' + operator : operator;
        } else if (operator === 'LAST' || operator === 'THIS' || operator === 'NEXT') {
            const custom: DateMomentInterval = clause as DateMomentInterval;
            let value = custom.operator + ' ' + custom.unit;
            if (custom.prefix) { value = custom.prefix + ' ' + value; }
            return value;
        } else if (operator === 'LASTN' || operator === 'NEXTN') {
            const custom: DateMomentNumberInterval = clause as DateMomentNumberInterval;
            ...
            if (custom.time) { value = value + ' ' + custom.time; }
            if (custom.prefix) { value = custom.prefix + ' ' + value; }
            return value;
        } 
        return '';
    ...
    private static dateRangeToString(operator: 'TO' | 'FOR', clause: DateRange): string {
