...
import { NumberCondition, NumberRange, NumberOperator, NumberRangeOperator, NumberClause, Clause } from './clause_types'
...
export class NumberParser extends BaseParser {
    ...
    private tokenize(): void {
        ...
        const params: TokenizerParams =
            { trimWordWhitespace: true, combineAdjacentWords: false, splitOnWhitespace: true, specialSubstrings, specialWords };
    ...
    private static groupClauses(clauses: NumberClause[]): NumberClause[] {
        ...
        for (let i = 0; i < clauses.length; i++) {
            ...
            if (clause.operator === 'range') {
                previous = undefined;
                outputs.push(clause);
            } else if (previous !== undefined && previous.operator === clause.operator) {
    ...
    private checkRange(token: Token, negated: boolean, clauses: NumberClause[], errors: FilterError[]): NumberClause[] {
        if (this.matchTokens(['[', 'word', ',', 'word', ']'])) {
            ...
            errors.push({ message: 'Invalid range expression', startIndex: token.startIndex, endIndex: token.endIndex });
            this.index++;
        }
        return clauses;
    }
    ...
    private consumeRange(startOperator: NumberRangeOperator, endOperator: NumberRangeOperator, clauses: NumberClause[], errors: FilterError[]): NumberClause[] {
        ...
        if (!NumberParser.isValidNumber(startValue)) {
            errors.push({ message: 'Invalid number', startIndex: startToken.startIndex, endIndex: startToken.endIndex });
        } else if (!NumberParser.isValidNumber(endValue)) {
            errors.push({ message: 'Invalid number', startIndex: endToken.startIndex, endIndex: endToken.endIndex });
        } else {
            const clause: NumberRange = { operator: 'range', startOperator: startOperator, startValue: startValue, endOperator: endOperator, endValue: endValue };
            clauses.push(clause);
        }
        this.index += 5;
        return clauses;
    ...
    private static getNumberOperator(tokenType: string): NumberOperator | undefined {
        switch (tokenType) {
            case '<=': return '<=';
            case '>=': return '>=';
            case '<': return '<';
    ...
    private static getNumberRnageOperator(tokenType: string): NumberRangeOperator | undefined {
        switch (tokenType) {
            case '<=': return '<=';
            case '>=': return '>=';
            case '<': return '<';
    ...
    private checkNumericExpression(tokenType: NumberOperator, clauses: NumberClause[]): boolean {
        if (this.getNext().type === tokenType && this.index < this.tokens.length - 1) {
            const numericValue: number = NumberParser.parseNumber(this.getAt(this.index + 1).value);
            if (!NumberParser.isValidNumber(numericValue)) {
                return false;
            }
            const clause: NumberCondition = { operator: tokenType, values: [numericValue] };
            clauses.push(clause);
            return true;
        }
        return false;
    ...
    private checkSimpleNumber(clauses: Clause[]): boolean {
        if (this.getNext().type === 'word') {
            const numericValue: number = NumberParser.parseNumber(this.getAt(this.index).value);
            if (!NumberParser.isValidNumber(numericValue)) {
                return false;
            }
