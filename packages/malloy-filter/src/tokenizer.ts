...
export class Tokenizer {
    ...
    private consumeSpecialSubstring(special: SpecialTokenMatch, tokens: Token[]): void {
        tokens.push({
            type: special.type,
            ...
            if (special.value instanceof RegExp) {
                ...
                if (regexp.test(token.value)) {
                    const value = special.ignoreCase
                        ...
                        : token.value;
                    return { type: special.type, value: value, startIndex: token.startIndex, endIndex: token.endIndex };
                }
            } else {
                const matches = special.ignoreCase
