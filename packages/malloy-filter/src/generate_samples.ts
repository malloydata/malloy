...
function testTokenizerString() {
    ...
    let params: TokenizerParams =
        { trimWordWhitespace: true, combineAdjacentWords: true, specialSubstrings, specialWords };
    let examples = [
        'CAT, DOG,mouse ',
        '-CAT,-DOG , -mouse',
        ' CAT,-"DOG",m o u s e',
...
function testParserSingle(str: string, parser: BaseParser, outputFormatter?: (clauses: Clause[]) => string): void {
...
function testSerializerRoundtrip(str: string, parser: BaseParser, serializerFunc: (clauses: Clause[]) => BaseSerializer): void {
    console.log('Input:  ' + str);
    try {
        const response = parser.parse();
...
function testNumberSerializer(): void {
    ...
    for (const example of numberExamples) {
        testSerializerRoundtrip(example, new NumberParser(example), (clauses) => new NumberSerializer(clauses));
    }
}

function testStringSerializer(): void {
    for (const example of stringExamples) {
        testSerializerRoundtrip(example, new StringParser(example), (clauses) => new StringSerializer(clauses));
    }
}
...
function testBooleanSerializer(): void {
    const examples = [
        [{ operator: 'TRUE' },]];
    for (const example of booleanExamples) {
        testSerializerRoundtrip(example, new BooleanParser(example), (clauses) => new BooleanSerializer(clauses));
    }
}
...
function testDateSerializer(): void {
    const examples = [
        ...
        [{ start: [{ type: 'day', value: 'today' }], operator: 'TO', end: [{ type: 'day', value: 'tomorrow' }] }],
        [{ type: 'day', value: 'today' }],
    ];
    for (const example of dateExamples) {
        testSerializerRoundtrip(example, new DateParser(example), (clauses) => new DateSerializer(clauses));
    }
}
