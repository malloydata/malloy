...
require("./is-sql-eq");
const sql_eq_1 = require("./sql-eq");
// No prebuilt shared model, each test is complete.  Makes debugging easier.
const dialects = [
    "bigquery",
    "postgres", //
];
const runtimes = new runtimes_1.RuntimeList(dialects);
afterAll(async () => {
...
runtimes.runtimeMap.forEach((runtime, databaseName) => {
    const sqlEq = (0, sql_eq_1.mkSqlEqWith)(runtime, { sql: basicTypes[databaseName] });
    ...
    describe(`time operations - ${databaseName}`, () => {
        describe(`time difference - ${databaseName}`, () => {
            ...
            test("seconds", async () => {
                const eq = sqlEq("seconds(@2001-01-01 00:00:00 to @2001-01-01 00:00:42)", "42");
                expect(await eq).isSqlEq();
            });
            test("many seconds", async () => {
                const eq = sqlEq("seconds(@2001-01-01 00:00:00 to @2001-01-02 00:00:42)", "86442");
                expect(await eq).isSqlEq();
            });
            test("minutes", async () => {
                const eq = sqlEq("minutes(@2001-01-01 00:00:00 to @2001-01-01 00:42:00)", "42");
                expect(await eq).isSqlEq();
            });
            test("many minutes", async () => {
                const eq = sqlEq("minutes(@2001-01-01 00:00:00 to @2001-01-02 00:42:00)", "1482");
                expect(await eq).isSqlEq();
            });
            test("hours", async () => {
                const eq = sqlEq("hours(@2001-01-01 00:00:00 to @2001-01-02 18:00:00)", "42");
                expect(await eq).isSqlEq();
            });
            test("days", async () => {
    ...
    describe(`join object - ${databaseName}`, () => {
        ...
        `)
                .run();
            expect(result.data.path(0, "t_month").value).toEqual(new Date("2021-02-01"));
        });
    });
});
//# sourceMappingURL=datetimes.spec.js.map