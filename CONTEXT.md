...
These use `scripts/femto-build.js`, a tiny content-hash-based build caching tool. Each package with codegen has a `codegen-config.json` with named targets specifying input globs and commands. femto-build hashes the inputs and skips the commands if nothing changed. This survives git operations (unlike Make's timestamp-based approach).
