...
| <pre> <code>CASE</code> <code>  WHEN size_n < 3 THEN 'S'</code> <code>  WHEN size_n <5 THEN 'M'</code> <code>ELSE 'L' END</code> </pre> | <pre> `size_n:` `  pick 'S' when < 3` `  pick 'M' when <5` `  else 'L' ` </pre> | Pick is Malloy’s improvement of SQL’s CASE statement. [Doc](https://looker-open-source.github.io/malloy/documentation/language/expressions.html#pick-expressions). Example below |
